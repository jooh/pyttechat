...
func TestSQLiteReplaceLastAssistantUpdatesOnlyFinalAssistant(t *testing.T) {
	store := newMigratedTestSQLite(t)
	ctx := context.Background()
	user := createTestUser(t, store, "drew")
	conversation, err := store.DefaultConversationForUser(ctx, user.ID)
	if err != nil {
		t.Fatalf("DefaultConversationForUser error = %v, want nil", err)
	}
	...
	if appendErr := store.AppendTurn(ctx, conversation.ID, llm.NewTextMessage(llm.RoleUser, "second"), llm.NewTextMessage(llm.RoleAssistant, "partial")); appendErr != nil {
		t.Fatalf("AppendTurn second error = %v, want nil", appendErr)
	}
	...
	assistant := llm.Message{
		Role: llm.RoleAssistant,
		Parts: []llm.Part{
			{Type: llm.PartReasoning, Summary: []string{"summary"}},
			{Type: llm.PartText, Text: "partial continued"},
		},
	}
	if err := store.ReplaceLastAssistant(ctx, conversation.ID, assistant); err != nil {
		t.Fatalf("ReplaceLastAssistant error = %v, want nil", err)
	}
	assistant.Parts[1].Text = "mutated"
	...
	if messages[0].Text() != "first" || messages[1].Text() != "answer one" || messages[2].Text() != "second" || messages[3].Text() != "partial continued" {
		t.Fatalf("messages = %#v, want final assistant replaced in place", messages)
	}
	if messages[3].Parts[0].Type != llm.PartReasoning || len(messages[3].Parts[0].Summary) != 1 {
		t.Fatalf("replacement assistant parts = %#v, want full parts JSON persisted", messages[3].Parts)
	}
}

func TestSQLiteReplaceLastAssistantRejectsInvalidInputWithoutPersisting(t *testing.T) {
	store := newMigratedTestSQLite(t)
	ctx := context.Background()
	user := createTestUser(t, store, "duncan")
	conversation, err := store.DefaultConversationForUser(ctx, user.ID)
	if err != nil {
		t.Fatalf("DefaultConversationForUser error = %v, want nil", err)
	}
	...
	for _, tc := range []struct {
		...
		{name: "missing conversation", conversationID: conversation.ID + 999, assistant: llm.NewTextMessage(llm.RoleAssistant, "replacement")},
		{name: "zero conversation", conversationID: 0, assistant: llm.NewTextMessage(llm.RoleAssistant, "replacement")},
		{name: "wrong role", conversationID: conversation.ID, assistant: llm.NewTextMessage(llm.RoleUser, "replacement")},
	} {
		if err := store.ReplaceLastAssistant(ctx, tc.conversationID, tc.assistant); !errors.Is(err, ErrInvalidArgument) {
			t.Fatalf("%s ReplaceLastAssistant error = %v, want ErrInvalidArgument", tc.name, err)
		}
	}
	...
	if len(messages) != 2 || messages[1].Text() != "answer" {
		t.Fatalf("messages after rejected replacement = %#v, want original assistant intact", messages)
	}
}

func TestSQLiteReplaceLastAssistantRejectsConversationEndingInUserMessage(t *testing.T) {
	store := newMigratedTestSQLite(t)
	ctx := context.Background()
	user := createTestUser(t, store, "duane")
	conversation, err := store.DefaultConversationForUser(ctx, user.ID)
	if err != nil {
		t.Fatalf("DefaultConversationForUser error = %v, want nil", err)
	}
...
VALUES (?, 1, 'user', '[{"type":"text","text":"question"}]', ?)
`, conversation.ID, formatTime(time.Now().UTC())); err != nil {
		t.Fatalf("insert user message error = %v, want nil", err)
	}

	err = store.ReplaceLastAssistant(ctx, conversation.ID, llm.NewTextMessage(llm.RoleAssistant, "answer"))
	if !errors.Is(err, ErrInvalidArgument) {
		t.Fatalf("ReplaceLastAssistant ending in user message error = %v, want ErrInvalidArgument", err)
	}
	...
	if len(messages) != 1 || messages[0].Role != llm.RoleUser || messages[0].Text() != "question" {
		t.Fatalf("messages after rejected replacement = %#v, want original user message intact", messages)
	}
}
